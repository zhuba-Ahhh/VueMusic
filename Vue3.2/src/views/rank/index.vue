          ...
          <div class="cover-info">
            <div class="cover-header">
              <div class="cover-title">
                ...
                <span
                  >({{
                    formartDate(rankInfo.updateTime, "MM月dd日")
                  }}
                  更新)</span
                >
              </div>
            ...
            <div class="cover-author-tags">
              <div class="cover-author" v-if="rankInfo.creator">
                ...
                <div class="cover-date">
                  {{ formartDate(rankInfo.createTime, "yyyy-MM-dd") }}
                </div>
              </div>
            </div>
            <div class="cover-digital">
              <span class="cover-playCount"
                ...
                {{ formartNum(rankInfo.playCount) }}次</span
              >
              <span class="cover-collect"
                ><i class="iconfont icon-collect"></i>
                {{ formartNum(rankInfo.subscribedCount) }}</span
              >
              <span class="cover-comment"
                ><i class="iconfont icon-comment"></i>
                {{ formartNum(rankInfo.commentCount) }}</span
              >
            </div>
...
import { reactive, onMounted, toRefs, watchEffect } from "vue";
import { useRoute, useRouter } from "vue-router";
import { ElMessage } from "element-plus";
import { topListDetail, listDetail } from "@/apis/modules";
import { formatSongs, formartNum, formartDate } from "@/utils/util";
...
const getTopListDetail = async () => {
  ...
  // 云音乐TOP榜
  info["listTop"] = res.list.filter((item: { ToplistType: any }) => {
    return item.ToplistType;
  });
  // 云音乐特色榜
  info["listFeature"] = res.list.filter(
    (item: { ToplistType: any; name: string | string[] }) => {
      return !item.ToplistType && item.name.indexOf("云音乐") >= 0;
    }
  );
  // 其他场景榜
  info["listOther"] = res.list.filter(
    (item: { ToplistType: any; name: string | string[] }) => {
      return !item.ToplistType && item.name.indexOf("云音乐") < 0;
    }
  );
...
const getListDetail = async () => {
  info["isLoading"] = true;
  const { data: res } = await listDetail({ id: info.rId, s: -1 });
...
const selectItem = (item: { id: number }) => {
  info.rId = item.id;
  router.push({ path: "rank", query: { type: info.type, rId: info.rId } });
};
...
onMounted(() => {
  info["type"] = (route.query.type as string) || info.type;
  info["rId"] = Number(route.query.rId as string);
  getTopListDetail();
});
