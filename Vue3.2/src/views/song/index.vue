...
import { songDetail, simiSong as SimiSong, simiPlayList } from "@/apis/modules";
import { formatSongs, formatSongTime } from "@/utils/util";
import { selectPlay } from "@/stores/methods";
...
const isCurSong = computed(
  () =>
    isPlayed.value &&
    curSongInfo.value &&
    Number(curSongInfo.value.id) === info.sId
);
// 当前播放状态
...
const getSimiSong = async () => {
  const { data: res } = await SimiSong({ id: "" + info.sId });
...
const getSimiPlayList = async () => {
  const { data: res } = await simiPlayList({ id: "" + info.sId });
...
onBeforeRouteUpdate((to) => {
  info["sId"] = Number(to.query.id);
  init();
});

onMounted(() => {
  info["sId"] = Number(route.query.id);
  init();
});
