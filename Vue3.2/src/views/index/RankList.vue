        ...
        <div v-for="item in top_list" :key="item.id" class="toplist_item">
          <div class="toplist_hd">
            ...
            <h5 class="toplist_update">
              最近更新：{{ formartDate(item.updateTime, "MM月dd日")
...
import { toplist, topRankList } from "@/apis/modules";
import { formatSongs, formartDate } from "@/utils/util";
import type { songType } from "@/types";
import { addList } from "@/stores/methods";
...
const getToplist = async () => {
  ...
  info["top_list"].forEach(async (item) => {
    const { data: res } = await topRankList({ id: item.id });

    info["songList"][item.id] = formatSongs(
...
// 添加当前歌曲到播放列表
const addSongList = (item: songType) => {
  addList([item]);
