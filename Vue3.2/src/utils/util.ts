export default {
  ...
  formartDate(originVal: Date, fmt: string) {
    ...
    for (let k in opt) {
      const ret: RegExpExecArray | null = new RegExp("(" + k + ")").exec(fmt);
      if (ret) {
  ...
  formatSongSecond(duration: string) {
    let arr: Array<number> = duration.split(":").map((item) => Number(item)),
      second: number = 0;
  ...
  // 歌曲毫秒格式化处理 03:30
  formatSongTime(duration: number = 0): string {
  ...
  // 添加歌曲到播放列表，过滤重复的歌曲
  concatPlayList(newList: Array<object> = [], oldList: Array<object> = []) {
    const arr: Array<object> = [...oldList, ...newList];
  ...
  // 处理歌曲
  formatSongs(list: string[], privileges: { [x: string]: { cp: any } }) {
    const ret: any[] = [];
    list.map((item, index) => {
      if (item.id) {
        // 是否有版权播放
