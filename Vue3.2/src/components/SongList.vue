...
const indexMethod = (page) => {
  if (!isScroll) {
    return (info.currentPage - 1) * info.pageSize + page + 1 + offset;
  } else {
    return page + 1;
  }
};
...
const curSongInfo = computed(() => {
  return playList.value[usePlayIndex.playIndex];
});
// 添加当前歌曲到播放列表
...
const delList = (index) => {
  if (playList.value.length > 1) {
    ...
    usePlayList.setPlayList(playList.value);
  } else {
    useIsPlayed.setPlayStatus(false);
    usePlayList.setPlayList([]);
  }
};
// 滚动到当前播放音乐的位置
const scrollCurSong = (cur) => {
  if (isScroll) {
    const curIndex = songList.findIndex((item) => {
      return item.id === cur.id;
    });

    if (curIndex > 7 && curIndex < songList.length - 8) {
      info.curScroll = -(curIndex - 4) * 50;
    } else if (curIndex >= songList.length - 8 && songList.length - 8 > 0) {
      info.curScroll = -(songList.length - 8) * 50;
    } else {
      info.curScroll = 0;
    }
    ...
    $curSongRef.addEventListener(
      ...
      (event) => {
        if (event.wheelDelta > 0 || event.detail < 0) {
          ...
          info.curScroll =
            Math.abs(info.curScroll) < ((songList.length - 8) / 2) * 100
              ? info.curScroll - 50
              : info.curScroll;
        }
      },
...
const isShowPagination = computed(() => {
  return songList.length > info.pageSize && !isScroll;
});
