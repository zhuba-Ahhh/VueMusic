...
const playSongStates = (state: any) => {
  audioRef.value.playAudioType(state);
};

// 歌曲播放类型：循环、单曲、随机
const playAudioMode = (mode: any) => {
  audioRef.value.playAudioMode(mode);
};

// 拖拽音量进度条
const setvolumeProgress = (progress: any) => {
  audioRef.value.setvolumeProgress(progress);
};
// 设置音量
const setVolumeHandler = (state: any) => {
