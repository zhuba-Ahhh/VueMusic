    ...
    <div class="wrapper">
      <div class="play-bar-inside">
        <div class="bar-l">
          ...
          <div class="bar-time">
            <span>{{ formatSongTime(Number(info.currentTime) * 1000) }}</span> /
...
const isShowPlayListTips = computed(() => {
  let val = useIsShowPlayList.isShowPlayListTips;

  if (val) {
    clearTimeout(info.tipsTimer);
    info.tipsTimer = setTimeout(() => {
      useIsShowPlayList.setIsShowPlayListTips(false);
    }, 3000);
  }
...
const audioProgressWidth = computed(() => {
  // 音频进度条长度
  return (
    (Number(info["currentTime"]) /
      formatSongSecond(curSongInfo.value.duration)) *
      100 +
    "%"
  );
});
...
const setAudioProgress = (params: { val: number; flag: any }) => {
  ...
  info["currentTime"] =
    params.val * formatSongSecond(curSongInfo.value.duration);
